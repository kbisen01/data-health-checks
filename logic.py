...
class FITSDataHealth:
    ...
    def __init__(self):
        ...
        self._load_rma_data()
        self._load_rma_operations()
        self._load_repair_status()
        self._load_current_fits_operation()
    ...
    def _load_process_table(self) -> None:
        """
        load process data from s3 bucket.
        """

        _process_file_location = "input/Process all.csv"
        _opn_map_location = "input/operations_mapping.csv"

        _process_columns: list = [
            "step_id",
            "serial_no",
            "workorder",
            "trans_seq",
        ...
        _process_parse_dates: list = ["datetime_checkin", "datetime_checkout"]

        self._process_df = pd.read_csv(
            S3Utils.get_s3_obj_body(_process_file_location),
            usecols=_process_columns,
            dtype=_process_types,
            parse_dates=_process_parse_dates,
        )
        self._process_df["serial_no"] = (
            self._process_df["serial_no"].str.split(".").str[0]
        )
        self._process_df = self._process_df.rename(columns=_process_rename)

        _operation_columns: list = ["operation", "OPN_Map"]
        _operation_types: dict = {"operation": str, "OPN_Map": str}

        # Add OPN_Map column to process_df
        _opn_map_df = pd.read_csv(
            S3Utils.get_s3_obj_body(_opn_map_location),
            usecols=_operation_columns,
            dtype=_operation_types,
        )
        self._process_df = self._process_df.merge(
            _opn_map_df, on="operation", how="left"
        )

        self._process_df["rma_number"] = extract_rma_from_wo(
            self._process_df["workorder"]
        )
        self._process_df["rma_number"] = dtype_rma(self._process_df["rma_number"])

    def _load_current_fits_operation(self) -> None:
        """
        load current FITS operation from s3 bucket.
        """

        _current_fits_operation_location = "input/process_current_operation.csv"
        _opn_map_location = "input/operations_mapping.csv"

        _current_fits_operation_columns: list = [
            "step_id",
            "serial_no",
            "workorder",
            "trans_seq",
        ...
        _current_fits_operation_rename: dict = {
            "serial_no": "serial_number",
        }
        _current_fits_operation_types: dict = {
            "step_id": int,
            "serial_no": str,
            "workorder": str,
            "trans_seq": int,
        ...
        _current_fits_operation_parse_dates: list = [
            "datetime_checkin",
            "datetime_checkout",
        ]

        self._current_fits_operation_df = pd.read_csv(
            S3Utils.get_s3_obj_body(_current_fits_operation_location),
            usecols=_current_fits_operation_columns,
            dtype=_current_fits_operation_types,
            parse_dates=_current_fits_operation_parse_dates,
        )
        self._current_fits_operation_df["serial_no"] = (
            self._current_fits_operation_df["serial_no"].str.split(".").str[0]
        )
        self._current_fits_operation_df = self._current_fits_operation_df.rename(
            columns=_current_fits_operation_rename
        )
        ...
        self._current_fits_operation_df = self._current_fits_operation_df.merge(
            _opn_map_df, on="operation", how="left"
        )

        self._current_fits_operation_df["rma_number"] = extract_rma_from_wo(
            self._current_fits_operation_df["workorder"]
        )
        self._current_fits_operation_df["rma_number"] = dtype_rma(
            self._current_fits_operation_df["rma_number"]
        )
        # self._current_fits_operation_df = self._current_fits_operation_df.rename(
        #     columns={
    ...
    def _load_parameter_table(self) -> None:
        ...
        """
        _parameter_file_location = "input/Parameter all.csv"

        _parameter_columns: list = [
            "parameter_id",
            "serial_no",
            "trans_seq",
            "sn_attr_code",
        ...
        _parameter_rename: dict = {"serial_no": "serial_number"}
        ...
        _parameter_parse_dates: list = ["datetime_script"]

        self._parameter_df = pd.read_csv(
            S3Utils.get_s3_obj_body(_parameter_file_location),
            usecols=_parameter_columns,
            dtype=_parameter_types,
            parse_dates=_parameter_parse_dates,
        )
        self._parameter_df["serial_no"] = (
            self._parameter_df["serial_no"].str.split(".").str[0]
        )
        self._parameter_df = self._parameter_df.rename(columns=_parameter_rename)

    def _load_rma_data(self) -> None:
        """
        ...
        _index_rma_dim_file_location = "input/RMA_dim.csv"
        _serial_file_location = "input/serialship.csv"

        _rma_dim_columns: list = [
            "id",
            "rma_number",
            "FA_location",
            "Serial_detail_id",
        ...
        _rma_dim_rename: dict = {"id": "RMA_dim_id"}
        _rma_dim_types: dict = {
            "id": int,
            "rma_number": "Int64",
            "FA_location": str,
        ...
        _rma_dim_parse_dates = []

        # serialship columns
        _serialship_columns: list = ["id", "serial_number"]
        _serialship_rename: dict = {"id": "Serial_detail_id"}
        _serialship_types: dict = {
            "id": int,
            "serial_number": str,
        }

        _serialship_parse_dates = []

        _rma_dim_df = pd.read_csv(
            S3Utils.get_s3_obj_body(_index_rma_dim_file_location),
            usecols=_rma_dim_columns,
            dtype=_rma_dim_types,
            parse_dates=_rma_dim_parse_dates,
        )
        _rma_dim_df = _rma_dim_df.rename(columns=_rma_dim_rename)
        _rma_dim_df = _rma_dim_df[
            _rma_dim_df["FA_location"].isin(_FABRINET_LOCATION_CODE)
        ]
        _rma_dim_df = _rma_dim_df[
            _rma_dim_df["created_from"] != _NON_RMA_CREATED_FROM_CODE
        ]

        _serialship_df = pd.read_csv(
            S3Utils.get_s3_obj_body(_serial_file_location),
            usecols=_serialship_columns,
            dtype=_serialship_types,
            parse_dates=_serialship_parse_dates,
        )
        _serialship_df = _serialship_df.rename(columns=_serialship_rename)
        _serialship_df["serial_number"] = (
            _serialship_df["serial_number"].str.split(".").str[0]
        )

        self._rma_data_df = _rma_dim_df.merge(
            _serialship_df, on="Serial_detail_id", how="left"
        )
        self._rma_data_df = self._rma_data_df.drop(
            columns=["FA_location", "Serial_detail_id", "created_from"]
        )

    def _load_rma_operations(self) -> None:
        """
        load RMA operations from s3 bucket.
        """
        _rma_operations_file_location = "input/rma_operations.csv"

        _rma_operations_columns: list = ["id", "process_status", "date", "RMA_dim_id"]

        _rma_operations_types: dict = {
            "process_status": str,
            "RMA_dim_id": int,
        }

        _rma_operation_parse_dates: list = ["date"]

        self._rma_operations_df = pd.read_csv(
            S3Utils.get_s3_obj_body(_rma_operations_file_location),
            usecols=_rma_operations_columns,
            dtype=_rma_operations_types,
            parse_dates=_rma_operation_parse_dates,
        )

    def _load_repair_status(self) -> None:
        """
        load repair status from s3 bucket.
        """
        _repair_status_file_location = "input/repair_status.csv"

        _repair_status_columns: list = ["id", "status", "date", "time", "RMA_dim_id"]

        _repair_status_types: dict = {
            "status": str,
            "RMA_dim_id": int,
        }
        ...
        self._repair_status_df = pd.read_csv(
            S3Utils.get_s3_obj_body(_repair_status_file_location),
            usecols=_repair_status_columns,
            dtype=_repair_status_types,
            parse_dates=_repair_status_parse_dates,
        )
        # Change time column
        self._repair_status_df["time"] = self._repair_status_df["time"].astype(
            "timedelta64"
        )
        self._repair_status_df["date"] = self._repair_status_df[
            "date"
        ] + self._repair_status_df["time"].fillna(pd.Timedelta(0))
        self._repair_status_df = self._repair_status_df.drop(columns=["time"])
    ...
    def check_all_repair_rma_present(self) -> "FITSDataHealth":
        temp = copy(self)
    ...
    def check_unknown_operation(self) -> "FITSDataHealth":
        temp = copy(self)
    ...
    def check_rma_portal_match(self) -> "FITSDataHealth":
        temp = copy(self)
    ...
    def check_rma_format(self) -> "FITSDataHealth":
        temp = copy(self)
        process_df = temp._process_df
    ...
    def check_oracle_shipped_units(self) -> "FITSDataHealth":
        temp = copy(self)
    ...
    def check_repair_close_fits_status(self) -> "FITSDataHealth":
        temp = copy(self)
    ...
    def check_trans_seq_in_param(self) -> "FITSDataHealth":
        temp = copy(self)
    ...
    def check_param_entry_closed_before_process_checkout(self) -> "FITSDataHealth":
        temp = copy(self)
        process_df = temp._process_df.loc[
            temp._process_df["operation"].isin(temp._DIAGNOSIS_OPERATION_CODE)
        ]
    ...
    # No New data added in process for last 3 days.
    ...
    def append_with_dh_process(self) -> "FITSDataHealth":
        """
